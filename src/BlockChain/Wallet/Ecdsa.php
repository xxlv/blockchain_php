 ...
 * this file taken from https://raw.githubusercontent.com/BitcoinPHP/BitcoinECDSA.php/master/src/BitcoinPHP/BitcoinECDSA/BitcoinECDSA.php
 */
...
class Ecdsa
{
    ...
    public function generateRandom256BitsHexaString ($extra = 'FkejkzqesrfeifH3ioio9hb55sdssdsdfOO:ss')
    {
        ...
        } while (gmp_cmp(gmp_init($res, 16),
                gmp_sub($this->n, gmp_init(1, 10))) === 1); // make sure the generate string is smaller than n
    ...
    public function getPubKeyPointsWithDerPubKey ($derPubKey)
    {
        if (substr($derPubKey, 0, 2) === '04' && strlen($derPubKey) === 130) {
            ...
            if ((substr($derPubKey, 0, 2) === '02' || substr($derPubKey, 0, 2) === '03') && strlen($derPubKey) === 66) {
    ...
    public function checkSignatureForRawMessage ($rawMessage)
    {
        ...
        preg_match_all("#\n-----BEGIN SIGNATURE-----\n(.{0,})\n(.{0,})\n-----END BITCOIN SIGNED MESSAGE-----#USi",
            $rawMessage, $out);
        $address = $out[1][0];
        $signature = $out[2][0];
